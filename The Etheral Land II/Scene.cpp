...
void Scene::Play(Player &Hero, sf::Sprite &sprite, sf::Sprite &PlayerSprite) {
    ...
    PlayerSprite.setPosition(60, 700./m_Scene[m_index].BackgroundSize.y*m_Scene[m_index].BackgroundSize.y-195);
}
...
void Scene::AddEffect(sf::Sprite &PlayerSprite, sf::Sprite &EffectSprite, Animator &animator, std::string effectName, int size) {
    ...
    EffectSprite.setPosition((PlayerSprite.getPosition().x-(EffectSprite.getLocalBounds().width*value.x-PlayerSprite.getLocalBounds().width)/2), PlayerSprite.getPosition().y-(EffectSprite.getLocalBounds().height*value.y-PlayerSprite.getLocalBounds().height)/2);
}
