...
void Game::Play(sf::RenderWindow &window, std::vector<sf::Texture> &Textures, sf::Font &Font, Player &Hero) {
    ...
    Animator KnightAnimator(Knight);
    Scene GameScene;
    
    // Load the textures
    mob.loadFromFile(resourcePath()+"RedKnight.png");
    
    auto& AnimatorKnight = KnightAnimator.CreateAnimation("KnightAnimator", mob, sf::seconds(2),true);
    AnimatorKnight.AddFrames(sf::Vector2i(0,0), sf::Vector2i(256,256), 13);
    AnimatorKnight.AddFrames(sf::Vector2i(0,256), sf::Vector2i(256,256), 12);
    
    Knight.setPosition(1000, PlayerSprite.getPosition().y+430);
    KnightAnimator.SwitchAnimation("KnightAnimator");
    ...
    Hero.Setup_Animations(PlayerAnimator);
    // Generate The HUD
    ...
    sf::Text opt1("- press space to awake", Font, 55);
    sf::Text opt2(Hero.recoverPlayer().name, Font, 75);
    ...
    while (window.isOpen())
    {
        ...
        KnightAnimator.update(deltaTime);
